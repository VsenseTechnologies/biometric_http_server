...
func(i *Init) InitilizeTables(w http.ResponseWriter , r *http.Request){
	if _ , err := i.db.Exec("CREATE TABLE admin(admin_id VARCHAR(100) PRIMARY KEY, admin_name VARCHAR(50) NOT NULL, password VARCHAR(100) NOT NULL)"); err != nil {
	...
	if _ , err := i.db.Exec("CREATE TABLE attendence(student_id VARCHAR(100), unit_id VARCHAR(50), date VARCHAR(20), login VARCHAR(20), logout VARCHAR(20), FOREIGN KEY (unit_id) REFERENCES biometric(unit_id) ON DELETE CASCADE , FOREIGN KEY (user_id) REFERENCES fingerprintdata(user_id) ON DELETE CASCADE)"); err != nil {
		w.WriteHeader(http.StatusConflict)
		json.NewEncoder(w).Encode(payload.SimpleFailedPayload{ErrorMessage: err.Error()})
		return
	}
