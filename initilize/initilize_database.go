...
func(i *Init) InitilizeTables(w http.ResponseWriter , r *http.Request){
	...
	if _ , err := i.db.Exec("CREATE TABLE fingerprintdata(student_id VARCHAR(100) PRIMARY KEY, unit_id VARCHAR(50) , fingerprint VARCHAR(1000), FOREIGN KEY (unit_id) REFERENCES biometric(unit_id) ON DELETE CASCADE)"); err != nil {
		w.WriteHeader(http.StatusConflict)
		json.NewEncoder(w).Encode(payload.SimpleFailedPayload{ErrorMessage: err.Error()})
		return
	}
